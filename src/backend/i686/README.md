...
32-bit encoder, producing ELFCLASS32 objects), and a builtin dynamic linker
(32-bit ELF, `.rel` relocations).
...
- **64-bit splitting**: All 64-bit operations split across `eax:edx` pairs
...
- **Linker**: Dynamic linking with PLT/GOT, 32-bit ELF, `.rel` (not `.rela`)
